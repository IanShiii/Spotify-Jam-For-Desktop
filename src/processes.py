...
def join_to_specific_user(host_token, joiner_token):
    host_is_active, host_track = get_current_track_info(host_token)
    joiner_is_active, joiner_track = get_current_track_info(joiner_token)
...
def sync(user1_token, user2_token, host_user):
    if host_user == 1:
        ...
        current_track_uri = user1_initial_track['uri']
        while True:
            new_values = update(user1_token, user2_token, is_playing, prev_song_uri, prev_progress, current_track_uri)
            is_playing = new_values[0]
            prev_song_uri = new_values[1]
            prev_progress = new_values[2]
            current_track_uri = new_values[3]
            sleep(.7)
    elif host_user == 2:
        ...
        current_track_uri = user2_initial_track['uri']
        while True:
            new_values = update(user1_token, user2_token, is_playing, prev_song_uri, prev_progress, current_track_uri)
            is_playing = new_values[0]
            prev_song_uri = new_values[1]
...
# return tuple (is_playing, prev_song_uri, prev_progress, current_track_uri)
def update(user1_token, user2_token, is_playing, prev_song_uri, prev_progress, initial_track_uri):
    print('loop')
    ...
    buffer_ms = 3000
    new_track_uri = initial_track_uri
    if user1_track['uri'] != user2_track['uri']:
    ...
    return is_playing, prev_song_uri, prev_progress, new_track_uri
